...
public class MainActivity extends AppCompatActivity implements JournalAdapter.ItemClickListener {
     ...
     * Often, this is after new data has been inserted through an AddEntryActivity,
    ...
    private void retrieveTasks() {
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                final List<JournalEntry> tasks = mDb.taskDao().loadAllEntries();
                // We will be able to simplify this once we learn more
                // about Android Architecture Components
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mAdapter.setEntries(tasks);
                    }
                });
            }
        });
    ...
    public void onItemClickListener(int itemId) {
        ...
        // COMPLETED (2) Launch AddEntryActivity with itemId as extra for the key AddEntryActivity.EXTRA_TASK_ID
